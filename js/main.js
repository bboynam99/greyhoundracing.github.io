//
var FOUNDATION_ADDRESS = 'TVaTfwWCuLncajJXbp53fCQ17BT1q2rzaJ';
var game_totalBet = 0;
...
async function componentDidMount() {
	...
	await $.getJSON("GreyhoundRacing.json", function(contracts) {
...
function userBetSend(betPlace, amount, callBack) {
	if(!amount){
	...
	contract.userBet(betPlace, sun).send({
		callValue: sun,
...
function getProfitWinRate() {
	contract.getMaxPrizeRate().call().then(function(r) {r=r/10;
		profit_WinRate = r;
...
function getBalanceUser(first) {
	tronWeb.trx.getBalance(tronWeb.defaultAddress.base58).then(function(r) {
		...
		if(window.s_oBetPanel)s_oBetPanel.updateCredit(START_CREDIT, first);
...
function serverToGameCallback(resultPlace){
	if(window.s_oBetPanel)s_oBetPanel.serverReturns(resultPlace);
	waitingDialog.hide('');
}
...
function addRecentData(data, watchE){
	...
	if(watchE){
		...
		if(window.s_oBetPanel)s_oBetPanel.serverReturns(result_place);
		waitingDialog.hide('');
	}
}
