function CForecastPanel(iX,iY,oParentContainer){
    ...
    this.unload = function(){
        for(var szKey in _aForecastBut){
            if(szKey.indexOf("forecast_") > -1)_aForecastBut[szKey].unload();
        }
    };
    //
    ...
    this._placeForecastBetForGreyhound = function(iGreyhoundIndex,iX,iY){
        ...
        for(var j=0;j<NUM_GREYHOUNDS;j++){
            if(j !== iGreyhoundIndex){
                var oBib1 = createSprite(oSpriteSheetBib,"bib_"+iGreyhoundIndex);
                oBib1.x = iX;
                oBib1.y = iY;
                oBib1.scaleX = oBib1.scaleY = iScale;
                _oContainer.addChild(oBib1);
                var oText = new createjs.Text("&", "16px " + PRIMARY_FONT, "#fff");
                oText.textAlign = "center";
                oText.textBaseline = "middle";
                oText.x = iX + (iWidthBib*iScale) + 10;
                oText.y = iY + (iHeightBib*iScale)/2;
                _oContainer.addChild(oText);
                var oBib2 = createSprite(oSpriteSheetBib,"bib_"+j);
                oBib2.x = oText.x + 10;
                oBib2.y = oBib1.y;
                ...
                var oTextOdd = new createjs.Text("x"+s_oGameSettings.getForecastOdd(iGreyhoundIndex,j), "18px " + TERTIARY_FONT, "#fff");
                oTextOdd.textAlign = "center";
                oTextOdd.textBaseline = "alphabetic";
                oTextOdd.x = oOddBg.x + oSpriteOddBg.width/2;
                oTextOdd.y = oOddBg.y + oSpriteOddBg.height - 2;
                _oContainer.addChild(oTextOdd);
                var oBut = new CButBet(oOddBg.x + oSpriteOddBg.width + (oSpriteButton.width*0.72)/2 + 5,oOddBg.y+ (oSpriteButton.height*0.72)/2,oSpriteButton,0.45,_oContainer, 2);
                oBut.setScale(0.72);
