function CGameSettings(oJsonData){
    ...
    this._init = function(oJsonData){
        ...
        _aGreyhoundNames = _oJsonData.greyhound_names;
        NUM_GREYHOUNDS = _aGreyhoundNames.length;
        this._initSimpleOdd();
    ...
    this._initSimpleOdd = function(){
        //STORE ODD INFOS
        _aGreyhoundWinOdd = _oJsonData.odd_win_bet;
        _aGreyhoundPlaceOdd = _oJsonData.odd_place_bet;
        _aGreyhoundShowOdd = _oJsonData.odd_show_bet;
    };
    //@
    this._initForecastOdd = function(){
        var aForecast = _oJsonData.forecast;
        _aForecastOdd = new Array();
        for(var i=0;i<NUM_GREYHOUNDS;i++)_aForecastOdd[i] = new Array();
        for(var k=0;k<aForecast.length;k++)_aForecastOdd[aForecast[k].first-1][aForecast[k].second-1] = aForecast[k].odd;
    };
    //@
    ...
    this.getIndexForFiches = function(iValue){
        ...
        for(var i=0;i<_aFichesValue.length;i++){
            if(_aFichesValue[i] === iValue)iRes = i;
        }
        return iRes; 
    };
    //@
    ...
    this.getRandomPath = function(){
        return _aPathSequence[Math.floor(Math.random()*_aPathSequence.length)];
    };
    //@
